...
SystemChatBot = [{
    "role": "system",
    "content": f"Hello, I am {os.environ['Username']}, You're a content writer. "
...
def Content(Topic):
  ...
  def ContentWriterAI(prompt):
        messages.append({"role": "user", "content": f"{prompt}"})  # Add the user
 ...
 # Function to serch for a topic on youtube.
...
# In PlayYoutube function.
def PlayYoutube(query):
    playonyt(query)  # Open search URL in browser
    return True  # Indicate success
...
def OpenApp(app, sess=requests.session()):
    try:
      ...
      def extract_links(html):            # Functions for Google search and link extraction (to be used when AppOpener fails)
        if html is None:
            return []
        soup = BeautifulSoup(html, 'html.parser')
        links = soup.find_all('a', {'jsname': 'UWckNb'})  # Specific link class to find search results
    ...
    # If the appopen fails, perform a Google search and open the first result
    html = search_google(app)
    if html:
        link = extract_links(html)[0]
        webopen(link)      # Open the first extracted link in the browser
...
def CloseApp(app):
    ...
    if "chrome" in app:  # Skip Chrome specifically
        pass
    else: 
        try:
...
async def TranslateAndExecute(commands: list[str]):
    funcs = []  # List to store async functions
    ...
    for command in commands:
        ...
        elif command.startswith("close"):  # Handle "play" commands
    ...
    for result in results:
        if isinstance(result, str):
            yield result  # Yield string results
        else:
            yield result 
